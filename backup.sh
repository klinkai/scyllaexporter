...
cat "${$SCYLLADB_KEYSPACE}.CQL.gz" | aws $AWS_ARGS s3 cp - s3://$S3_BUCKET/$S3_PREFIX/${SCYLLADB_KEYSPACE}_$(date +"%Y-%m-%dT%H:%M:%SZ").CQL.gz || exit 2

rm -rf "${$SCYLLADB_KEYSPACE}.CQL.gz"
